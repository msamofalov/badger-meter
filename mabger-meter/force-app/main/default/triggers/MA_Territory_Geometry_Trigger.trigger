trigger MA_Territory_Geometry_Trigger on sma__MATerritoryGeometry__c (before insert, before update, after insert, after update) {
    ...
    if (!System.isBatch() && trigger.isAfter && (trigger.isInsert || trigger.isUpdate)){
        ...
        for (sma__MATerritoryGeometry__c geo : trigger.new){
